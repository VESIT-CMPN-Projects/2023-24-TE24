...
if (selected == 'Heart Disease Screening'):
...
# creating a button for Prediction
if st.button('Heart Disease Test Result'):
    # Check if input values are not empty before converting to float
    if all(value != '' for value in [age, sex, cp, trestbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal]):
        # Convert input to a NumPy array for prediction
        input_data = np.array([[float(age), float(sex), float(cp), float(trestbps), float(chol), float(fbs), float(restecg),
